...
- `INS-260628-CB50` [high] Diminishing returns on a coding agent kick in around 40% of context-window utilization, so the goal is a small, high-signal context, not a full one.
- `INS-260626-BC5E` [high] A 'Grill Me' skill that makes the AI ask dozens of questions until you share a design concept beats default plan mode, which is too eager to produce an asset.
...
- `INS-260627-805E` [medium] You cannot write enough rules to encode your taste — agents should learn it from watching you edit their output, not from ever-growing config files.
- `INS-260627-4E4A` [medium] You can parallelize and pipeline agents, but you can't fully automate building good software, because the path to a good product is curved and only walkable by iterating with taste.
