...
- `INS-260627-BDF8` [medium] The bottleneck for beating a frontier model on a specific task is no longer compute or data but the ability to define a verifiable reward and wrap it in an environment.
...
- `INS-260628-32EF` [medium] Providers rate-limit to keep each user within a cache-hit-rate band that keeps GPUs efficient, so a token subscription is really a purchase of cache KB slots in token storage.
...
- `INS-260626-01AD` [medium] Do not hard-code provider speed knobs without a capability check and migration path.
...
- `INS-260627-F805` [medium] A model router should optimize quality per constrained resource, not simply call the highest-ranked model more often.
