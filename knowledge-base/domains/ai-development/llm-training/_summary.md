...
- `INS-260329-5D48` [high] Imbalanced datasets create deceptively high accuracy scores while systematically failing on underrepresented classes.
...
- `INS-260329-8407` [high] Features measured on different scales (0.03 vs 100s) cause distance calculations to be dominated by the larger-scaled features.
...
- `INS-260329-D357` [high] A model can be 82% accurate overall while completely failing on one class — precision and recall reveal these hidden failure patterns.
...
- `INS-260403-5FD1` [high] DeepSeek R1 achieves o1-level reasoning using pure RL instead of supervised fine-tuning with worked examples.
...
- `INS-260329-F854` [high] KNN achieved 82% accuracy on the gamma/hadron classification task, competitive with more complex models trained on the same data.
...
- `INS-260329-9524` [high] Splitting data into train/validate/test sets prevents models from memorizing answers instead of learning patterns.
