---
id: INS-260324-D612
domain: startups
topic: business-models
title: >-
  Pizza's unit economics explain why independents thrive where burger joints
  can't — low cost, low labor, small footprint
actionability: reference
confidence: high
shelf_life: evergreen
status: active
tags:
  - unit-economics
  - pizza
  - food-business
  - independent-vs-chain
sources:
  - type: youtube
    title: How Moneyball Ruined Your Favorite Snacks
    author: Modern MBA
    url: 'https://www.youtube.com/watch?v=evArab8oOoo'
date_extracted: '2026-03-24'
last_accessed: null
access_count: 0
indexed: true
embedded: true
embedding_model: nomic-embed-text
resolutions:
  one_line: >-
    Pizza has the lowest food costs in fast food (flour, canned tomatoes,
    subsidized cheese), needs minimal space, and 50% of US pizza shops are still
    independent — unlike burgers where 90% are chains.
  standard: >-
    Pizza is 'more factory than restaurant' — assembly lines built to produce
    one product with minor variations. Core ingredients face less inflation than
    meat and fresh produce. Pizzerias take up substantially smaller real estate
    than competitors. This creates a uniquely level playing field: 1 in 2 US
    pizza spots is independent (vs 1 in 10 for burgers). The big three
    (Domino's, Pizza Hut, Papa John's) trade margins for volume with $10 large
    pizza deals that no burger chain can match. Domino's averages only 11%
    margin but scales through franchises; it's really a logistics company that
    sells ingredients at markup to franchisees.
related:
  - INS-260324-C2B9
  - INS-260327-8EB7
  - INS-260330-420D
  - INS-260327-2DE8
  - INS-260327-048B
  - INS-260327-D5F0
stance: >-
  Pizza has the lowest food costs in fast food (flour, canned tomatoes,
  subsidized cheese), needs minimal space, and 50% of US pizza shops are still
  ...
---
Pizza is 'more factory than restaurant' — assembly lines built to produce one product with minor variations. Core ingredients face less inflation than meat and fresh produce. Pizzerias take up substantially smaller real estate than competitors. This creates a uniquely level playing field: 1 in 2 US pizza spots is independent (vs 1 in 10 for burgers). The big three (Domino's, Pizza Hut, Papa John's) trade margins for volume with $10 large pizza deals that no burger chain can match. Domino's averages only 11% margin but scales through franchises; it's really a logistics company that sells ingredients at markup to franchisees.
