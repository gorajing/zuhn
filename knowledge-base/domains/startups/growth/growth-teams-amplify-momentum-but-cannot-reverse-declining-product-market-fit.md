---
id: INS-260404-B893
domain: startups
topic: growth
title: Growth teams amplify momentum but cannot reverse declining product-market fit
actionability: immediate
confidence: high
shelf_life: evergreen
status: active
tags:
  - growth-teams
  - product-market-fit
  - organizational-design
  - hiring-timing
sources:
  - type: blog
    title: >-
      10 growth tactics that never work | Elena Verna (Amplitude, Miro,
      SurveyMonkey, Dropbox)
    author: >-
      {"@type":"Organization","name":"Lenny's
      Newsletter","url":"https://www.lennysnewsletter.com","description":"Deeply
      researched product, growth, and career advice—newsletter, podcast,
      community, and living
      library","interactionStatistic":{"@type":"InteractionCounter","name":"Subscribers","interactionType":"https://schema.org/SubscribeAction","userInteractionCount":1000000},"identifier":"pub:10845","logo":{"@type":"ImageObject","url":"https://substackcdn.com/image/fetch/$s_!8MSN!,f_auto,q_auto:good,fl_progressive:steep/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2F441213db-4824-4e48-9d28-a3a18952cbfc_592x592.png","contentUrl":"https://substackcdn.com/image/fetch/$s_!8MSN!,f_auto,q_auto:good,fl_progressive:steep/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2F441213db-4824-4e48-9d28-a3a18952cbfc_592x592.png","thumbnailUrl":"https://substackcdn.com/image/fetch/$s_!8MSN!,w_128,h_128,c_limit,f_auto,q_auto:good,fl_progressive:steep/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2F441213db-4824-4e48-9d28-a3a18952cbfc_592x592.png"},"image":{"@type":"ImageObject","url":"https://substackcdn.com/image/fetch/$s_!8MSN!,f_auto,q_auto:good,fl_progressive:steep/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2F441213db-4824-4e48-9d28-a3a18952cbfc_592x592.png","contentUrl":"https://substackcdn.com/image/fetch/$s_!8MSN!,f_auto,q_auto:good,fl_progressive:steep/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2F441213db-4824-4e48-9d28-a3a18952cbfc_592x592.png","thumbnailUrl":"https://substackcdn.com/image/fetch/$s_!8MSN!,w_128,h_128,c_limit,f_auto,q_auto:good,fl_progressive:steep/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2F441213db-4824-4e48-9d28-a3a18952cbfc_592x592.png"},"sameAs":["https://twitter.com/lennysan"]}
    url: >-
      https://www.lennysnewsletter.com/p/10-growth-tactics-that-never-work-elena-verna
date_extracted: '2026-04-04'
last_accessed: null
access_count: 0
indexed: true
embedded: true
embedding_model: nomic-embed-text
resolutions:
  one_line: >-
    Growth teams are multipliers of existing traction, not creators of it — fix
    the product before hiring growth.
  standard: >-
    When core metrics are declining, companies instinctively hire growth leaders
    expecting a turnaround. Elena Verna argues this is a category error: growth
    teams optimize and amplify existing product-market fit signals through
    experimentation and channel development. They cannot fix fundamental product
    problems, competitive displacement, or eroding PMF.


    The practical implication is sequencing — address the root cause (product
    quality, positioning, competitive response) before layering growth.
    Premature growth hiring wastes the hire's credibility and the company's
    runway. The prerequisite checklist is clear: strong PMF, at least $1M ARR,
    enough user data for meaningful experiments, and self-serve revenue for PLG
    companies.
stance: >-
  A growth team hired into a business with declining core metrics will fail
  because growth functions can only multiply existing signal, not manufacture it
  from zero.
related:
  - INS-260323-CB34
  - INS-260327-C673
  - INS-260330-8920
  - INS-260403-A14B
  - INS-260405-A2A2
  - INS-260403-39CF
  - PRI-260406-AFA7
  - PRI-260323-9D46
  - INS-260405-443C
---
When core metrics are declining, companies instinctively hire growth leaders expecting a turnaround. Elena Verna argues this is a category error: growth teams optimize and amplify existing product-market fit signals through experimentation and channel development. They cannot fix fundamental product problems, competitive displacement, or eroding PMF.

The practical implication is sequencing — address the root cause (product quality, positioning, competitive response) before layering growth. Premature growth hiring wastes the hire's credibility and the company's runway. The prerequisite checklist is clear: strong PMF, at least $1M ARR, enough user data for meaningful experiments, and self-serve revenue for PLG companies.
