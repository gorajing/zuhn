---
id: INS-260404-F8AF
domain: startups
topic: growth
title: Growth models must be deliberately evolved on an 18-month cycle
actionability: immediate
confidence: medium
shelf_life: evergreen
status: active
tags:
  - growth-loops
  - diminishing-returns
  - channel-diversification
  - resource-allocation
sources:
  - type: blog
    title: >-
      10 growth tactics that never work | Elena Verna (Amplitude, Miro,
      SurveyMonkey, Dropbox)
    author: >-
      {"@type":"Organization","name":"Lenny's
      Newsletter","url":"https://www.lennysnewsletter.com","description":"Deeply
      researched product, growth, and career advice—newsletter, podcast,
      community, and living
      library","interactionStatistic":{"@type":"InteractionCounter","name":"Subscribers","interactionType":"https://schema.org/SubscribeAction","userInteractionCount":1000000},"identifier":"pub:10845","logo":{"@type":"ImageObject","url":"https://substackcdn.com/image/fetch/$s_!8MSN!,f_auto,q_auto:good,fl_progressive:steep/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2F441213db-4824-4e48-9d28-a3a18952cbfc_592x592.png","contentUrl":"https://substackcdn.com/image/fetch/$s_!8MSN!,f_auto,q_auto:good,fl_progressive:steep/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2F441213db-4824-4e48-9d28-a3a18952cbfc_592x592.png","thumbnailUrl":"https://substackcdn.com/image/fetch/$s_!8MSN!,w_128,h_128,c_limit,f_auto,q_auto:good,fl_progressive:steep/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2F441213db-4824-4e48-9d28-a3a18952cbfc_592x592.png"},"image":{"@type":"ImageObject","url":"https://substackcdn.com/image/fetch/$s_!8MSN!,f_auto,q_auto:good,fl_progressive:steep/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2F441213db-4824-4e48-9d28-a3a18952cbfc_592x592.png","contentUrl":"https://substackcdn.com/image/fetch/$s_!8MSN!,f_auto,q_auto:good,fl_progressive:steep/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2F441213db-4824-4e48-9d28-a3a18952cbfc_592x592.png","thumbnailUrl":"https://substackcdn.com/image/fetch/$s_!8MSN!,w_128,h_128,c_limit,f_auto,q_auto:good,fl_progressive:steep/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2F441213db-4824-4e48-9d28-a3a18952cbfc_592x592.png"},"sameAs":["https://twitter.com/lennysan"]}
    url: >-
      https://www.lennysnewsletter.com/p/10-growth-tactics-that-never-work-elena-verna
date_extracted: '2026-04-04'
last_accessed: null
access_count: 0
indexed: true
embedded: true
embedding_model: nomic-embed-text
resolutions:
  one_line: >-
    Dedicate 20-25% of growth resources to exploring new channels because
    current tactics will hit diminishing returns within 18 months.
  standard: >-
    Elena Verna prescribes a specific cadence for growth model evolution: layer
    new channels on top of existing ones, expect diminishing returns from
    current tactics, and allocate 20-25% of resources to exploring new growth
    loops. New channels take 6-18 months before showing meaningful results, so
    exploration must start well before current channels plateau.


    This counters two common failure modes: complacency (riding one channel
    until it dies) and panic-switching (abandoning a working channel for the
    next shiny thing). The 20-25% allocation creates a portfolio approach — the
    majority of resources optimize proven channels while a meaningful minority
    builds the next generation. The 18-month cycle gives enough time to validate
    new loops without betting the company on unproven channels.
stance: >-
  Every growth channel faces diminishing returns, so companies that don't
  allocate 20-25% of resources to exploring new growth loops will plateau within
  18 months.
related:
  - INS-260329-AF18
  - INS-260403-ADB3
  - PRI-260406-46C3
  - INS-260330-78B1
  - PRI-260406-1A4D
  - PRI-260403-E097
---
Elena Verna prescribes a specific cadence for growth model evolution: layer new channels on top of existing ones, expect diminishing returns from current tactics, and allocate 20-25% of resources to exploring new growth loops. New channels take 6-18 months before showing meaningful results, so exploration must start well before current channels plateau.

This counters two common failure modes: complacency (riding one channel until it dies) and panic-switching (abandoning a working channel for the next shiny thing). The 20-25% allocation creates a portfolio approach — the majority of resources optimize proven channels while a meaningful minority builds the next generation. The 18-month cycle gives enough time to validate new loops without betting the company on unproven channels.
