---
id: INS-260404-899D
domain: startups
topic: growth-strategy
title: Owned and earned channels compound while rented channels commoditize
actionability: immediate
confidence: high
shelf_life: evergreen
status: active
tags:
  - growth-channels
  - virality
  - word-of-mouth
  - paid-acquisition
  - defensibility
sources:
  - type: blog
    title: >-
      10 growth tactics that never work | Elena Verna (Amplitude, Miro,
      SurveyMonkey, Dropbox)
    author: >-
      {"@type":"Organization","name":"Lenny's
      Newsletter","url":"https://www.lennysnewsletter.com","description":"Deeply
      researched product, growth, and career advice—newsletter, podcast,
      community, and living
      library","interactionStatistic":{"@type":"InteractionCounter","name":"Subscribers","interactionType":"https://schema.org/SubscribeAction","userInteractionCount":1000000},"identifier":"pub:10845","logo":{"@type":"ImageObject","url":"https://substackcdn.com/image/fetch/$s_!8MSN!,f_auto,q_auto:good,fl_progressive:steep/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2F441213db-4824-4e48-9d28-a3a18952cbfc_592x592.png","contentUrl":"https://substackcdn.com/image/fetch/$s_!8MSN!,f_auto,q_auto:good,fl_progressive:steep/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2F441213db-4824-4e48-9d28-a3a18952cbfc_592x592.png","thumbnailUrl":"https://substackcdn.com/image/fetch/$s_!8MSN!,w_128,h_128,c_limit,f_auto,q_auto:good,fl_progressive:steep/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2F441213db-4824-4e48-9d28-a3a18952cbfc_592x592.png"},"image":{"@type":"ImageObject","url":"https://substackcdn.com/image/fetch/$s_!8MSN!,f_auto,q_auto:good,fl_progressive:steep/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2F441213db-4824-4e48-9d28-a3a18952cbfc_592x592.png","contentUrl":"https://substackcdn.com/image/fetch/$s_!8MSN!,f_auto,q_auto:good,fl_progressive:steep/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2F441213db-4824-4e48-9d28-a3a18952cbfc_592x592.png","thumbnailUrl":"https://substackcdn.com/image/fetch/$s_!8MSN!,w_128,h_128,c_limit,f_auto,q_auto:good,fl_progressive:steep/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2F441213db-4824-4e48-9d28-a3a18952cbfc_592x592.png"},"sameAs":["https://twitter.com/lennysan"]}
    url: >-
      https://www.lennysnewsletter.com/p/10-growth-tactics-that-never-work-elena-verna
date_extracted: '2026-04-04'
last_accessed: null
access_count: 0
indexed: true
embedded: true
embedding_model: nomic-embed-text
resolutions:
  one_line: >-
    Prioritize owned channels like virality and UGC over rented channels like
    paid social, because competitors can always outbid you on rented platforms.
  standard: >-
    Elena Verna draws a sharp distinction between rented channels (SEO, SEM,
    paid social) where you pay for access to someone else's audience, and
    owned/earned channels (virality, word of mouth, user-generated content) that
    create compounding, defensible growth. Rented channels suffer from the Law
    of Shitty Clickthroughs — performance degrades as competitors flood in and
    platforms extract more value.


    The strategic implication is resource allocation: rather than pouring budget
    into channels where you're bidding against every competitor, invest in
    product mechanics that generate organic distribution. Miro's Miroverse (user
    templates) and Dropbox's referral loops are examples. These channels take
    longer to build but create moats that paid acquisition never can.
stance: >-
  Companies that prioritize virality, word-of-mouth, and UGC over SEO, SEM, and
  paid social build defensible growth engines that competitors cannot replicate.
related:
  - PRI-260406-7D93
  - INS-260405-543B
  - INS-260330-7251
  - INS-260327-F2DF
  - PRI-260328-BDC2
---
Elena Verna draws a sharp distinction between rented channels (SEO, SEM, paid social) where you pay for access to someone else's audience, and owned/earned channels (virality, word of mouth, user-generated content) that create compounding, defensible growth. Rented channels suffer from the Law of Shitty Clickthroughs — performance degrades as competitors flood in and platforms extract more value.

The strategic implication is resource allocation: rather than pouring budget into channels where you're bidding against every competitor, invest in product mechanics that generate organic distribution. Miro's Miroverse (user templates) and Dropbox's referral loops are examples. These channels take longer to build but create moats that paid acquisition never can.
