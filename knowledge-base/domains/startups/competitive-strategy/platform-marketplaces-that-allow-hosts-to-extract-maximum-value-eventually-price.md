---
id: INS-260327-624F
domain: startups
topic: competitive-strategy
title: >-
  Platform marketplaces that allow hosts to extract maximum value eventually
  price out the customers who created demand
actionability: immediate
confidence: high
shelf_life: evergreen
status: active
tags:
  - consumer-culture
  - market-dynamics
sources:
  - type: youtube
    title: The DEATH of Airbnb
    author: Levi Hildebrand
    url: 'https://www.youtube.com/watch?v=hgkHApmWnJM'
date_extracted: '2026-03-27'
last_accessed: null
access_count: 0
indexed: true
embedded: true
embedding_model: nomic-embed-text
resolutions:
  one_line: >-
    When marketplace platforms allow suppliers to continuously increase pricing
    and add fees, they eventually price out the 
  standard: >-
    When marketplace platforms allow suppliers to continuously increase pricing
    and add fees, they eventually price out the customer segment that created
    the demand, destroying the value proposition.


    This pattern reflects fundamental dynamics in consumer psychology and
    competitive strategy.
stance: >-
  When marketplace platforms allow suppliers to continuously increase pricing
  and add fees, they eventually price out the customer segment that created the
  demand, destroying the value proposition.
related:
  - INS-260324-157A
  - INS-260327-31A7
  - INS-260329-9F8D
  - INS-260403-5D89
  - INS-260403-1C25
  - INS-260329-6079
  - INS-260402-C883
---
When marketplace platforms allow suppliers to continuously increase pricing and add fees, they eventually price out the customer segment that created the demand, destroying the value proposition.

This pattern reflects fundamental dynamics in consumer psychology and competitive strategy.
