---
id: INS-260403-83E2
domain: startups
topic: competitive-strategy
title: The wealth tax as existential threat to the startup equity model
actionability: reference
confidence: medium
shelf_life: time-sensitive
status: active
tags:
  - wealth-tax
  - unrealized-gains
  - startup-equity
  - california
  - regulatory-risk
sources:
  - type: blog
    title: SEIU Delenda Est
    author:
      - '@type': Person
        name: Scott Alexander
        url: 'https://substack.com/@astralcodexten'
        description: null
        identifier: 'user:12009663'
        image:
          '@type': ImageObject
          contentUrl: >-
            https://substackcdn.com/image/fetch/$s_!HTBd!,f_auto,q_auto:good,fl_progressive:steep/https%3A%2F%2Fbucketeer-e05bbc84-baa3-437e-9518-adb32be77984.s3.amazonaws.com%2Fpublic%2Fimages%2F7b500d22-1176-42ad-afaa-5d72bc36a809_44x44.png
          thumbnailUrl: >-
            https://substackcdn.com/image/fetch/$s_!HTBd!,w_128,h_128,c_limit,f_auto,q_auto:good,fl_progressive:steep/https%3A%2F%2Fbucketeer-e05bbc84-baa3-437e-9518-adb32be77984.s3.amazonaws.com%2Fpublic%2Fimages%2F7b500d22-1176-42ad-afaa-5d72bc36a809_44x44.png
    url: 'https://www.astralcodexten.com/p/seiu-delenda-est'
date_extracted: '2026-04-03'
last_accessed: null
access_count: 0
indexed: false
embedded: false
resolutions:
  one_line: >-
    California's proposed billionaire tax on unrealized gains could force
    founders to sell equity to pay taxes on illiquid paper wealth.
  standard: >-
    The proposed California wealth tax includes unrealized gains and values
    company stakes by voting rights rather than ownership percentage. This means
    a typical founder who maintains board control despite dilution could be
    taxed on more wealth than they actually own. Since startup founders
    routinely hold billions in paper wealth for years before any liquidity
    event, this would force premature equity sales or restructuring.


    The retroactive provision — applying to anyone who was a California resident
    in January, despite not being voted on until November — compounds the
    problem by preventing orderly planning. Even if the tax fails at the ballot
    box or in courts, the threat alone has driven 20 of 21 surveyed California
    tech billionaires to develop exit plans, potentially causing the very
    capital flight that reduces state revenue regardless of the measure's
    outcome.
stance: >-
  Taxing unrealized gains on private company stakes would break the fundamental
  startup financing model where founders hold illiquid paper wealth for years
  before any cash event.
---
The proposed California wealth tax includes unrealized gains and values company stakes by voting rights rather than ownership percentage. This means a typical founder who maintains board control despite dilution could be taxed on more wealth than they actually own. Since startup founders routinely hold billions in paper wealth for years before any liquidity event, this would force premature equity sales or restructuring.

The retroactive provision — applying to anyone who was a California resident in January, despite not being voted on until November — compounds the problem by preventing orderly planning. Even if the tax fails at the ballot box or in courts, the threat alone has driven 20 of 21 surveyed California tech billionaires to develop exit plans, potentially causing the very capital flight that reduces state revenue regardless of the measure's outcome.
