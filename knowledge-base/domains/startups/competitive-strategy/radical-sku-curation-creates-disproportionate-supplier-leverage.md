---
id: INS-260403-6670
domain: startups
topic: competitive-strategy
title: Radical SKU curation creates disproportionate supplier leverage
actionability: reference
confidence: high
shelf_life: evergreen
status: active
tags:
  - curation
  - supply-chain
  - negotiation-leverage
  - costco
sources:
  - type: blog
    title: How to Do Great Work
    url: 'https://paulgraham.com/greatwork.html'
date_extracted: '2026-04-03'
last_accessed: null
access_count: 0
indexed: true
embedded: true
embedding_model: nomic-embed-text
resolutions:
  one_line: >-
    Costco stocks 4,000 SKUs vs Walmart's 140,000, making each slot fiercely
    contested by suppliers willing to cut prices.
  standard: >-
    Conventional retail wisdom says more selection serves more customers. Costco
    inverts this: by stocking only 4,000 items (vs 30,000 at a typical
    supermarket or 140,000 at Walmart Supercenter), each product slot becomes
    enormously valuable to suppliers. Companies compete aggressively to be the
    one or two choices in their category, accepting lower margins and even
    reengineering products (like the cashew container that saved 24,000
    pallets/year). This creates a virtuous cycle: fewer SKUs → more volume per
    SKU → stronger negotiating position → lower prices → higher renewal rates.
    The counterintuitive lesson is that constraint in selection can be a source
    of power rather than a limitation.
stance: >-
  Offering dramatically fewer product choices per category gives retailers more
  negotiating power per SKU than broad-selection competitors, because suppliers
  compete intensely for scarce shelf space.
related:
  - INS-260404-6BA2
  - INS-260403-180D
  - INS-260403-0DC2
  - INS-260404-4B5D
  - INS-260403-E143
---
Conventional retail wisdom says more selection serves more customers. Costco inverts this: by stocking only 4,000 items (vs 30,000 at a typical supermarket or 140,000 at Walmart Supercenter), each product slot becomes enormously valuable to suppliers. Companies compete aggressively to be the one or two choices in their category, accepting lower margins and even reengineering products (like the cashew container that saved 24,000 pallets/year). This creates a virtuous cycle: fewer SKUs → more volume per SKU → stronger negotiating position → lower prices → higher renewal rates. The counterintuitive lesson is that constraint in selection can be a source of power rather than a limitation.
